class Kolibri
{
  ...
  void Color2()
  {
    for(int i = 0;i< 14;i++)
    {
        vasen[i].changeColor(100);
        oikea[i].changeColor(100);  
    }
    
    for(int i = 0;i< 9;i++)
    {
        torso[i].changeColor(100);
    }
    for(int i = 0;i< 3;i++)
    { 
      pyrsto[i].changeColor(100);
    }
    for(int i = 0;i< 2;i++)
    {
      paa[i].changeColor(100);
    }
  ...
  void fly1(int sync)
  {
     for(int i = 0;i < 14;i++)
    {
      vasen[i].setPosition(vasen[i].headX()+sync*3,vasen[i].headY()+sync*3,vasen[i].headZ()*sync*5,vasen[i].tailX(),vasen[i].tailY(),vasen[i].tailZ());
      oikea[i].setPosition(oikea[i].headX()-sync*3,oikea[i].headY()+sync*3,oikea[i].headZ()*sync*5,oikea[i].tailX(),oikea[i].tailY(),oikea[i].tailZ());
    }
  }
